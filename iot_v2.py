...
if __name__ == "__main__":
    ...
    image_list = glob.glob(f"{FOLDER_PATH}/*.jpg") + glob.glob(f"{FOLDER_PATH}/*.png")
    
    if not image_list:
        print(f"No images found in '{FOLDER_PATH}'! Please add images.")
    else:
        print(f"--- Starting test: {len(image_list)} images in {NETWORK_ENV} environment ---")
        
        for i, img_path in enumerate(image_list, 1):
            print(f"[{i}/{len(image_list)}] Transferring...: {os.path.basename(img_path)}")
            run_inference_test(img_path, network_label=NETWORK_ENV)
